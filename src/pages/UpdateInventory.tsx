import Grid from "@mui/material/Grid";
import Typography from "@mui/material/Typography";
import { styled, useTheme } from "@mui/material/styles";
import DradAndDropUploads from "src/components/inventory/DragnDropUploads";
import ErrorOutlineIcon from "@mui/icons-material/ErrorOutline";
import useTitle from "src/hooks/useTitle";
import { Container } from "src/components/common/styles/PageContainerStyles";
import { ErrorsList, ErrorMsg } from "src/utilityStyles/pagesUtilityStyles";
import { useTypedSelector } from "src/hooks/useTypedSelector";

const ContentWrapper = styled(Grid)({
  minHeight: "80vh",
  background: "#fff",
  display: "flex",
  justifyContent: "center",
});

const UploadBoxWrapper = styled(Grid)(({ theme }) => ({
  padding: "2rem 5rem",

  [theme.breakpoints.only("xs")]: {
    padding: "2rem",
  },
}));

const UpdateInventory = () => {
  useTitle("Admin : Update Inventory");
  const theme = useTheme();

  const loadingProductAction = useTypedSelector(
    (state) => state.products.loadingProductAction
  );
  const error = useTypedSelector((state) => state.products.error);

  return (
    <Container container direction="column">
      <Grid item container pb={2}>
        <Typography variant="h3" color={theme.palette.secondary.dark}>
          Update Inventory
        </Typography>
      </Grid>
      <ContentWrapper item container alignItems="center" direction="column">
        <Grid item>
          <Typography variant="h4" align="center" gutterBottom>
            Attach document below
          </Typography>
          <Typography variant="body2" align="center" color="dimgrey">
            Note: Only Excel spreadsheet file is allowed
          </Typography>
        </Grid>
        {!loadingProductAction && error && (
          <Grid item>
            <ErrorsList
              item
              container
              component="ul"
              justifyContent="center"
              alignItems="center"
              columnGap={1}
            >
              <ErrorOutlineIcon color="error" />
              <ErrorMsg variant="body2" color="error" component="li">
                {error}
              </ErrorMsg>
            </ErrorsList>
          </Grid>
        )}
        <UploadBoxWrapper item container>
          <DradAndDropUploads />
        </UploadBoxWrapper>
      </ContentWrapper>
    </Container>
  );
};

export default UpdateInventory;
